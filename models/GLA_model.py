...
You can specify '--model GLA' to use this model.
...
class GLAModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--add_tps', action='store_true', default = False, help='if specified, add tps transformation')
        ...
        parser.add_argument('--add_GLU', action='store_true', default = True, help='if specified, add GLU-net')        
        ...
        parser.add_argument('--n_layers_feat_extract', type=int, default=3, help='# layers in feater extraction of GLA')
    ...
    def __init__(self, opt):
        ...
        self.model_names = ['GLA']
        
        # define networks; you can use opt.isTrain to specify different behaviors for training and test.
        self.netGLA = networks.define_GLA(opt.input_nc_A, opt.input_nc_B, opt.ngf, opt.n_layers_feat_extract, opt.img_height, opt.img_width, opt.grid_size, opt.add_tps, opt.add_depth,opt.add_GLU, opt.add_segmt, opt.norm, opt.use_dropout, opt.init_type, opt.init_gain, self.gpu_ids)

        if self.isTrain:  # only defined during training time
            # define loss functions.
            self.criterionWarp = torch.nn.L1Loss()
            self.criterionGrid = networks.GridLoss(opt.img_height, opt.img_width)
            self.criterionVGG = networks.VGGLoss(device='cuda')
            '''
            if self.add_depth:
            ...
            if self.use_theta_loss:
                self.criterionTheta = networks.ThetaLoss(grid_size=9, device=self.device)
            '''
            if self.use_grid_loss:
                self.criterionGrid = networks.GridLoss(opt.img_height, opt.img_width)

            # define and initialize optimizers. You can define one optimizer for each network.
            self.optimizer = torch.optim.Adam(self.netGLA.parameters(), lr=opt.lr, betas=(0.5, 0.999))
    ...
    def forward(self):
        ...
        self.output = self.netGLA(self.agnostic, self.c)
        ...
        if self.output['grid_tps'] is not None:
            ...
            map_x = self.grid_tps[:,:,:,0]
            map_y = self.grid_tps[:,:,:,1]
            self.warped_cloth = self.tps_cloth = kornia.geometry.transform.remap(self.c,map_x,map_y,mode='bilinear', padding_mode='zeros', align_corners=True, normalized_coordinates=True)
            self.warped_cloth_mask = self.tps_cloth_mask =kornia.geometry.transform.remap(self.cm,map_x,map_y,mode='bilinear', padding_mode='zeros', align_corners=True, normalized_coordinates=True)
            self.tps_grid = kornia.geometry.transform.remap(self.im_g,map_x,map_y,mode='bilinear', padding_mode='zeros', align_corners=True, normalized_coordinates=True)
            
            '''
            self.c = self.c.cpu().detach().numpy()
            self.im_g = self.im_g.cpu().detach().numpy()
            self.cm =self.cm.cpu().detach().numpy()
            h_scale=self.grid_tps.shape[2]
            w_scale=self.grid_tps.shape[3]
            self.grid_tps = self.grid_tps.cpu().detach().numpy()
            ...
            self.tps_cloth = self.tps_cloth.to('cuda')
            self.im = self.im.to('cuda')
            
            self.warped_overlay = (self.tps_cloth + self.im) * 0.5 # just for visual
            self.warped_test = self.tps_cloth*self.warped_cloth_mask+self.im*(1-self.warped_cloth_mask)
    ...
    def backward(self):
        ...
        self.loss_gla = torch.tensor(0.0, requires_grad=True).to(self.device)
        #self.warped_cloth = self.warped_cloth.to('cuda')
        #self.im_c = self.im_c.to('cuda')
        
        if self.add_GLU:
            self.loss_warp = self.opt.lambda_warp * self.criterionWarp(self.warped_cloth, self.im_c)
            '''
            EPE_map = torch.norm(output_test-self.grid_tps, 2, 1)
            batch_size = EPE_map.size(0)
            self.loss_warp = EPE_map.sum()/batch_size
            self.loss_warp = EPE_map.sum()/163840
            '''
            self.loss_grid = self.opt.lambda_grid * self.criterionGrid(self.grid_tps)
            self.loss_grid = 40 * self.loss_grid / (self.grid_tps.shape[0] * self.grid_tps.shape[1] * self.grid_tps.shape[2])

            #self.loss_gla = self.loss_warp + self.loss_grid
            
            self.loss_vgg = 1.0 * self.criterionVGG(self.warped_cloth, self.im_c)
            
            
            self.loss_gla = self.loss_warp + self.loss_grid + self.loss_vgg
            

        '''
        if self.use_theta_loss:
            self.loss_theta = self.opt.lambda_theta * self.criterionTheta(self.theta_tps)
            self.loss_gla += self.loss_theta
        
        

        if self.use_grid_loss:
            self.loss_grid = self.opt.lambda_grid * self.criterionGrid(self.grid_tps)
            self.loss_gla += self.loss_grid
        '''

        self.loss_gla.backward()
    
    def optimize_parameters(self):
        """Update network weights; it will be called in every training iteration."""
        self.forward() # compute (fake) output from G      
        self.optimizer.zero_grad()  # clear network GLA's existing gradients
        self.backward()              # calculate gradients for network GLA
        self.optimizer.step()        # update gradients for network GLA
