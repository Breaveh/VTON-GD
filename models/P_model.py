...
You can specify '--model P' to use this model.
...
class PModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--warproot', type=str, default='results/aligned/SGN/test_pairs', help='path to SGN warping result folder')
        ...
        parser.add_argument('--input_nc', type=int, default=7, help='input nc for P generator. [base: 7| input segmt: +1 | input depth: +1]')
        parser.add_argument('--output_nc', type=int, default=4, help='output nc for P generator')
    ...
    def __init__(self, opt):
        ...
        if self.input_depth:
            opt.input_nc += 1
        self.netP = networks.define_P(opt.input_nc, opt.output_nc, opt.num_downs, opt.ngf, opt.norm, opt.use_dropout, opt.init_type, opt.init_gain, self.gpu_ids)
        if self.isTrain:
        ...
        if self.isTrain:  # only defined during training time
            ...
            self.optimizer_G = torch.optim.Adam(self.netP.parameters(), lr=opt.lr, betas=(0.5, 0.999))
    ...
    def optimize_parameters(self):
        ...
        self.optimizer_G.zero_grad() # clear network P's existing gradients
