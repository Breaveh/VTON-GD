...
You can specify '--model SGN' to use this model.
...
class SGNModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--n_layers_feat_extract', type=int, default=3, help='# layers in feater extraction of SGN')
    ...
    def __init__(self, opt):
        ...
        self.model_names = ['SGN']
        
        # define networks; you can use opt.isTrain to specify different behaviors for training and test.
        self.netSGN = networks.define_SGN(opt.input_nc_A, opt.input_nc_B, opt.ngf, opt.n_layers_feat_extract, opt.img_height, opt.img_width, opt.grid_size, opt.add_tps, opt.add_depth, opt.add_segmt, opt.norm, opt.use_dropout, opt.init_type, opt.init_gain, self.gpu_ids)
        ...
        if self.isTrain:  # only defined during training time
            ...
            self.optimizer = torch.optim.Adam(self.netSGN.parameters(), lr=opt.lr, betas=(0.5, 0.999))
    ...
    def forward(self):
        ...
        self.output = self.netSGN(self.agnostic, self.c)
    ...
    def backward(self):
        ...
        self.loss_sgn = torch.tensor(0.0, requires_grad=True).to(self.device)

        if self.add_tps:
            self.loss_warp = self.opt.lambda_warp * self.criterionWarp(self.warped_cloth, self.im_c)
            self.loss_sgn += self.loss_warp
        ...
        if self.add_depth:
            ...
            self.loss_sgn += (self.loss_fdepth + self.loss_bdepth)
        
        if self.add_segmt:
            self.loss_segmt = self.opt.lambda_segmt * self.criterionSegmt(self.segmt_pred, self.segmt_gt.squeeze(1))
            self.loss_sgn += self.loss_segmt

        if self.use_theta_loss:
            self.loss_theta = self.opt.lambda_theta * self.criterionTheta(self.theta_tps)
            self.loss_sgn += self.loss_theta

        if self.use_grid_loss:
            self.loss_grid = self.opt.lambda_grid * self.criterionGrid(self.grid_tps)
            self.loss_sgn += self.loss_grid


        self.loss_sgn.backward()
    
    def optimize_parameters(self):
        """Update network weights; it will be called in every training iteration."""
        self.forward() # compute (fake) output from G      
        self.optimizer.zero_grad()  # clear network SGN's existing gradients
        self.backward()              # calculate gradients for network SGN
        self.optimizer.step()        # update gradients for network SGN
