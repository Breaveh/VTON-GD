...
You can specify '--model RDG' to use this model.
...
class RDGModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        Returns:
            the modified parser.
        """
        parser.add_argument('--warproot', type=str, default='results/aligned/SGN/test_pairs', help='path to SGN result folder')
        parser.add_argument('--input_gradient', action='store_true', default=True, help='if specified, add image (sobel) gradient to input')
        parser.add_argument('--input_nc', type=int, default=8, help='input nc for RDG generator [base: 8 | input gradient: + 4]')
        parser.add_argument('--output_nc', type=int, default=2, help='output nc for RDG generator')
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['rdg', 'fdepth', 'bdepth']
        if self.use_grad_loss:
        ...
        if self.isTrain and self.use_gan_loss:
            self.model_names = ['RDG', 'FND', 'BND']
        else:
            self.model_names = ['RDG']
        

        # define networks; you can use opt.isTrain to specify different behaviors for training and test.
        if self.input_gradient:
            opt.input_nc += 4
        self.netRDG = networks.define_RDG(opt.input_nc, opt.output_nc, opt.ngf, opt.norm, opt.init_type, opt.init_gain, self.gpu_ids)
        ...
        if self.isTrain:  # only defined during training time
            ...
            self.optimizer_G = torch.optim.Adam(self.netRDG.parameters(), lr=opt.lr, betas=(0.5, 0.999))
    ...
    def forward(self):
        ...
        outputs = self.netRDG(self.input)
    ...
    def backward_G(self):
        ...
        self.loss_rdg = self.loss_fdepth + self.loss_bdepth
        ...
        if self.use_grad_loss:
            ...
            self.loss_rdg += self.loss_fgrad + self.loss_bgrad
        ...
        if self.use_normal_loss:
            ...
            self.loss_rdg += self.loss_fnormal + self.loss_bnormal
        ...
        if self.use_gan_loss: # G(fake_input) should fake the discriminator
            ...
            self.loss_rdg += self.loss_fgan + self.loss_bgan

        self.loss_rdg.backward()
