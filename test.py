...
if __name__ == '__main__':
    ...
    for i, data in enumerate(dataset):
        ...
        if 'GLA' in opt.model: # save warped_cloth, warped_cloth_mask, warp_grid, roi_segmt and roi_depth to disk
            if opt.add_GLU:
                warped_cloth = tensor2im(model.warped_cloth)
                warped_grid = tensor2im(model.tps_grid)
                warped_overlay = tensor2im(model.warped_overlay)
                warped_test = tensor2im(model.warped_test)
                ...
                save_image(warped_overlay, os.path.join(results_dir, 'warp-grid', c_name.replace('.jpg','_overlay.jpg')))
                save_image(warped_test, os.path.join(results_dir, 'warp-grid', c_name.replace('.jpg','_test.jpg')))
                # save cloth sobel
        ...
        if 'P' in opt.model: # save p_tryon to disk
            save_image(tensor2im(model.p_tryon), os.path.join(results_dir, 'tryon', im_name))
            save_image(tensor2im(model.im), os.path.join(results_dir, 'person', im_name))
        
        if 'RDG' in opt.model: # save refined depth to disk
