...
class Palette(BaseModel):
    ...
    def set_input(self, data):
        ...
        '''
        self.im_name = data['im_name']
        self.len = len(self.im_name)
        self.path = self.im_name                       # meta info
        self.c_name = data['c_name']                          # meta info
        self.im = self.set_device(data.get('person'))                       # for ground truth
        self.tryon = self.set_device(data.get('tryon'))
        self.mask = self.set_device(data.get('mask'))
        self.mask_body = self.set_device(data.get('mask_body'))
        self.mask_before = self.set_device(data.get('mask_before'))
        self.tryon_ = self.set_device(data.get('tryon_'))
        self.body = self.set_device(data.get('body_forview'))
        self.cond_image = self.tryon
        self.gt_image = self.im
    ...
    def save_current_results(self):
        ...
        for idx in range(self.batch_size):
            ...
            ret_result.append(self.im[idx].detach().float().cpu())

            ret_path.append('body_{}'.format(self.path[idx]))
            ret_result.append(self.body[idx].detach().float().cpu())

            ret_path.append('Maskbody_{}'.format(self.path[idx]))
            ret_result.append(self.mask_body[idx].detach().float().cpu())
            ...
            ret_path.append('Maskbefore_{}'.format(self.path[idx]))
            ret_result.append(self.mask_before[idx].detach().float().cpu())
            ...
            for val_data in tqdm.tqdm(self.val_loader):
                ...
                if self.opt['distributed']:
                    if self.task in ['inpainting','uncropping']:
                        ...
                        #self.output = self.output*self.mask_body + self.tryon*(1-self.mask_body)
                        #self.visuals = self.visuals*self.mask_body + self.tryon*(1-self.mask_body)
                    else:
        ...
        with torch.no_grad():
            for phase_data in tqdm.tqdm(self.phase_loader):
                ...
                if self.opt['distributed']:
                    if self.task in ['inpainting','uncropping']:
                        self.output, self.visuals = self.netG.module.restoration(self.cond_image, y_t=self.cond_image, 
                            y_0=self.tryon_, mask=self.mask, sample_num=self.sample_num)
